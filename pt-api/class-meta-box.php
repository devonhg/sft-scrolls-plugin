...
class SFT_PLUG_pt_meta {
	...
	//Hook into the appropriate actions when the class is constructed.
		public function __construct($title, $desc, $pt, $hide = false , $type = "text", $options = null) {
			...
			SFT_PLUG_pt_meta::$instances[] = $this; 
	            ...
	            $post_types = $this->pt; //limit meta box to certain post types
		            if (is_array($this->pt)){
			            if ( in_array( $post_type, $post_types )) {
							add_meta_box(
								$this->id
								,__( $this->title, 'SFT_PLUG_textdomain' )
	...
	//Render Meta Box Content
		public function render_meta_box_content( $post ) {
			...
			_e( "<sub>" . $this->desc . "</sub><br>" , 'SFT_PLUG_textdomain' );
			echo '</label> ';
	        ...
	        if ( $this->type == "media" ){
	        	...
	        	if ( $value !== ""){
	        		echo "<div>";
						echo SFT_PLUG_func::media_check( $value );
					echo "</div>";
	        	}
